...
const BMIChart: React.FC<BMIChartProps> = ({ bmiData }) => {
  if (!bmiData || bmiData.length === 0) {
    return (
      <View className="items-center justify-center">
        <Text className="text-gray-500">Không có dữ liệu BMI</Text>
      </View>
    );
  }
            ...
            return (
              <Svg>
                {values.map((value, index) => {
                  const x = (index + 1) * xSpacing + 15; // Tọa độ X của điểm
